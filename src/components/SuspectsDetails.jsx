...
const SuspectsDetails = (props) => {
    ...
    return (
        <div className='p-6 card w-72 bg-base-200 shadow-xl rounded-2xl'>
            <h1>Suspect #{props.index + 1}</h1>
            <div className='flex flex-col gap-y-2 gap-x-8 my-4'>
                <div className='flex items-baseline justify-start gap-x-4'>
                    <h1 className='text-md font-semibold'>Suspect Name:</h1>
                    <p className='text-md'>{props.SuspectName ? props.SuspectName : "Not Specified"}</p>
                </div>
                <div className='flex items-baseline justify-start gap-x-4'>
            ...
            {role === "admin" && <div className='flex gap-x-4'>
                <Link to={`/suspects/edit-suspect/${props.CrimeId}/${props.SuspectId}`} className='btn btn-info btn-square'><FontAwesomeIcon icon={faEdit} /></Link>
                <button onClick={() => document.getElementById('my_modal_1').showModal()} className='btn btn-error'><FontAwesomeIcon icon={faTrashAlt} /></button>
            </div>}
